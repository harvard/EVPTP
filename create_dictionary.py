...
def parse_date(dateToParse):
    ...
    :return: a separated year and term (e.g., 2004 and FALL) as strings
    '''
    year = dateToParse[:-1]
...
def pickle_and_save(course_data_dict, outf):
    ...
    :param course_data_dict: the dictionary to pickle
    :param outf: the outf to which we should write the pickled dictionary
...
def unpickle_and_print(from_file):
    ...
    :param from_file: the file from which to read the dictionary
...
if __name__ == '__main__':
    ...
    # if a previous dictionary has already been constructed, build a set so we don't add duplicates
    if os.path.exists(out_file):
        course_data_dict, course_set = unpickle_and_build_set(out_file)
    # otherwise, initialize the data structures yourself
    else:
        course_data_dict = {}
        course_set = set()
